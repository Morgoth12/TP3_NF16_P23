...
void rechercheProduits(T_Magasin *magasin, float prix_min, float prix_max) {
    ...
    T_Rayon *rayon_temp = creerRayon("temp");     // Crée un rayon temporaire vide
    ...
    while (rayon_courant != NULL) {
        ...
        while (produit_courant != NULL) {
            ...
            if (prix_produit >= prix_min && prix_produit <= prix_max) {
                ajouterProduit(rayon_temp,produit_courant->designation, produit_courant->prix, produit_courant->quantite_en_stock);
                trouve_produit = 1;
            }
